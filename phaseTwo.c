...
void drawPath(int prevX, int prevY, int futureX, int futureY) {
  int i;
  for (i = 0; i < 5; i++) {
    addNode(prevX + (float)(i/4.0) * (futureX - prevX), prevY + (float)(i/4.0) * (futureY - prevY));
    set_point(prevX + (float)(i/4.0) * (futureX - prevX), prevY + (float)(i/4.0) * (futureY - prevY));
  }
}

void setPath(int currentX, int currentY, int nextX, int nextY) {
  int i; static int prevx = 0; static int prevy = 0;
  int targetX, targetY;
  if (nextX != currentX) {
    drawPath(prevx, prevy, nextX * 60 - (nextX - currentX) * 30, (1 + nextY) * 60);
    prevx = nextX * 60 - (nextX - currentX) * 30;
    prevy = (1 + nextY) * 60;
  } else {
    drawPath(prevx, prevy, nextX * 60, (1 + nextY) * 60 - (nextY - currentY) * 30);
    prevx = nextX * 60;
    prevy = (1 + nextY) * 60 - (nextY - currentY) * 30;
  }
...
void followBack() {
  ...
  while(1) {
    target = findTarget();
    if (abs(xCurrent - 180) < 25 && abs(yCurrent - 240) < 15) { //10 is too low
      set_motors(0,0);
      exit(0);
    }
...
int racePace() {
  ...
  set_origin();
	...
	// }
 //  (walls + 16)->xCoord = 1; (walls + 16)->yCoord = 6;
 //  (walls + 17)->xCoord = 3; (walls + 17)->yCoord = 2;
 //  (walls + 18)->xCoord = 3; (walls + 18)->yCoord = 4;
 //  (walls + 19)->xCoord = 4; (walls + 19)->yCoord = 1;
 //  (walls + 20)->xCoord = 4; (walls + 20)->yCoord = 5;
 //  (walls + 21)->xCoord = 6; (walls + 21)->yCoord = 3;
